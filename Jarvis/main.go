...
func main() {
	...
	s.AddTool(mcp.NewTool("restart_profiles",
		mcp.WithDescription("Restarts the MCPM daemon container to reload all MCP profiles with updated configurations. Use this after editing server configs (API keys, commands, etc.) to apply changes without restarting clients. Profiles run as SSE services inside Docker."),
		mcp.WithString("profile",
			mcp.Description("Optional: restart only a specific profile (e.g., 'p-pokeedge', 'memory'). If not specified, restarts all profiles.")),
	), handleRestartProfiles)
