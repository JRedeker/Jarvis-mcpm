	...
	return []ToolDefinition{
		// System Management
		{
			Tool: mcp.NewTool("check_status",
				mcp.WithDescription("Comprehensive system health check for MCPM, Docker, and all services. Validates Node.js, Python, dependencies, running containers, and SSE endpoints. Returns actionable fix suggestions for any issues found."),
			),
		...
		},
		{
			Tool: mcp.NewTool("edit_server",
				...
				mcp.WithString("url",
					mcp.Description("New URL for SSE/HTTP servers"),
				),
		...
		// Client Management
		{
			Tool: mcp.NewTool("manage_client",
				mcp.WithDescription("Configure MCP client applications (Claude Code, Claude Desktop, etc.) with profiles and servers."),
				mcp.WithString("action",
					mcp.Description("Action: 'ls', 'edit', 'import', 'config'"),
					mcp.Required(),
				),
				mcp.WithString("client_name",
					mcp.Description("Client to configure (e.g., 'claude-code', 'codex')"),
				),
