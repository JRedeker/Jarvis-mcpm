...
**Version:** 3.1.0 (Diagnostic Edition)
**Language:** Go 1.24+
...
### Consolidated Tool Architecture (v3.1)
...
| `jarvis_diagnose` | profile_health, test_endpoint, logs, full | MCP debugging (NEW in v3.1) |
...
### `jarvis_diagnose` (MCP Debugging) - NEW in v3.1
...
## 🎯 Version 3.1: Diagnostic Edition

Building on v3.0's context efficiency, v3.1 adds essential diagnostic capabilities for AI agents.

### Key Changes in v3.1

- **New Tool:** `jarvis_diagnose` for debugging MCP profile issues
- **HTTP Transport Default:** Production now auto-detects HTTP API for better performance
- **Actionable Errors:** All tools now provide next-step suggestions on failure
- **Profile Health:** Real-time visibility into MCP profile subprocess status
