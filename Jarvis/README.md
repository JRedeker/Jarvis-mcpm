...
**Version:** 5.1.0 (Enhanced Operations Edition)
**Language:** Go 1.24+
...
*   **`handlers/`**: Tool handlers with dependency injection for testability
    *   `server.go` - Consolidated tool definitions (9 tools in v3.1)
    *   `consolidated.go` - Action-based routing handlers
    *   `handlers.go` - Core business logic implementations (incl. diagnostic handlers)
    *   `http_mcpm.go` - HTTP API transport for MCPM
    *   `opencode.go` - OpenCode client integration
...
### Consolidated Tool Architecture (v5.1)

In v3.0, Jarvis consolidated 24 tools into 8 action-based tools for **52% context token reduction**.
In v3.1, we added `jarvis_diagnose` for debugging MCP profile issues.
In v5.1, we added enhanced Docker operations, test runner, and config backup capabilities.
...
### `jarvis_diagnose` (MCP Debugging)
Essential for AI agents to self-diagnose when MCP tools fail to load.
*   **Actions:** `profile_health`, `test_endpoint`, `logs`, `full`, `config_sync`
*   **Use Case:** When tools/list returns errors or profiles fail to start
*   **Example Workflow:**
    1.  `jarvis_diagnose(action="profile_health")` - Check supervisor status
    2.  `jarvis_diagnose(action="logs", profile="qdrant")` - Get subprocess errors
    3.  `jarvis_diagnose(action="test_endpoint", endpoint="http://localhost:6279/mcp")` - Test MCP protocol
    4.  `jarvis_diagnose(action="full")` - Comprehensive diagnostic report

### `jarvis_system` (Enhanced Docker Operations) - NEW in v5.1
Full Docker lifecycle management without shell access.
*   **Actions:** `rebuild`, `stop`, `start`, `docker_logs`, `docker_status`, `build`
*   **Parameters:** `service` (target container), `lines` (log lines), `no_cache` (rebuild), `component` (build target)
*   **Example:**
    ```javascript
    jarvis_system({ action: "rebuild", no_cache: true })  // Full rebuild
    jarvis_system({ action: "docker_logs", service: "mcp-daemon", lines: 100 })
    jarvis_system({ action: "build", component: "jarvis" })  // Build Jarvis binary
    ```

### `jarvis_project` (Test Runner) - NEW in v5.1
Run tests directly with auto-detection of project type.
*   **Actions:** `test`
*   **Parameters:** `verbose`, `package`, `project_type`
*   **Supported:** Go, Python, Node, TypeScript
*   **Example:**
    ```javascript
    jarvis_project({ action: "test" })  // Auto-detect and run
    jarvis_project({ action: "test", verbose: true, package: "./handlers/..." })
    ```

### `jarvis_config` (Config Backup) - NEW in v5.1
Export and import MCPM configurations for backup/restore.
*   **Actions:** `export`, `import`
*   **Parameters:** `path`, `include_secrets`
*   **Features:** Secret scrubbing, automatic backup before import
*   **Example:**
    ```javascript
    jarvis_config({ action: "export", path: "backup.json" })  // Scrubs secrets
    jarvis_config({ action: "import", path: "backup.json" })  // Creates .bak files
    ```
...
Building on v5.0's Micro-Profile architecture, v5.1 eliminates shell command gaps for AI agents.

### Key Changes in v5.1

- **Docker Operations:** `rebuild`, `stop`, `start`, `docker_logs`, `docker_status`, `build` actions
- **Test Runner:** Auto-detect project type and run tests via `jarvis_project(action="test")`
- **Config Backup:** Export/import MCPM configurations with secret scrubbing
- **Enhanced Logging:** `jarvis_diagnose` logs now use `docker compose logs` for better output
- **31 New Tests:** Comprehensive coverage for all new functionality

### Version History

| Version | Focus | Key Change |
|:--------|:------|:-----------|
| v5.1 | Enhanced Operations | Docker control, test runner, config backup |
| v5.0 | Micro-Profiles | Composable profile architecture |
| v3.1 | Diagnostics | Added `jarvis_diagnose`, HTTP transport default |
| v3.0 | Context Efficiency | Consolidated 24 → 8 tools (52% token reduction) |
| v2.x | Feature Complete | Full MCPM integration |
